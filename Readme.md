...
- Subir una captura o PDF de la consulta (por ejemplo un print de patentechile.com) para que el sitio intente extraer los
  valores automáticamente con OCR (Tesseract.js + pdf.js). También admite el flujo manual si prefieres completar a mano. El
  lector OCR reconoce tablas como la de "Información vehicular" aunque cada fila quede en líneas separadas (ej. `Patente` en
  una línea y el valor en la siguiente).
- Si pegas texto plano desde la página oficial o desde un OCR externo, pulsa **Analizar texto y rellenar ficha** junto al campo
  de notas para intentar completar automáticamente la información vehicular antes de guardar.
...
Si no ves los últimos cambios en Pages (por ejemplo, sigues viendo el fondo claro), fuerza un *hard refresh* (Ctrl+Shift+R) o edita las query strings de los assets en `index.html` (por ejemplo `styles.css?v=2024-06-29` y `script.js?v=2024-06-29`) para obligar al navegador a descargar la versión nueva. Cada vez que subas cambios, aumenta el número de versión para evitar que el navegador siga usando archivos antiguos.
...
4. **Versión de assets**: en `index.html` actualiza las query strings de CSS/JS y la constante `APP_VERSION` en `script.js` al mismo valor (ej. `2024-06-29`) antes de hacer push.
