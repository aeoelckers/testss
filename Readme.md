...
Si no ves los últimos cambios en Pages (por ejemplo, sigues viendo el fondo claro), fuerza un *hard refresh* (Ctrl+Shift+R) o edita las query strings de los assets en `index.html` (por ejemplo `styles.css?v=2024-06-21` y `script.js?v=2024-06-21`) para obligar al navegador a descargar la versión nueva. Cada vez que subas cambios, aumenta el número de versión para evitar que el navegador siga usando archivos antiguos.
...
4. **Versión de assets**: en `index.html` actualiza las query strings de CSS/JS y la constante `APP_VERSION` en `script.js` al mismo valor (ej. `2024-06-21`) antes de hacer push.
